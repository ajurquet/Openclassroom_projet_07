...
def bruteforce(depense_max, donnees, lst_actions_selectionees=[]):
    """
    Algorithme de force brute, essayant toutes les combinaisons possible
    """

    if donnees:

        # val1 et lst_val1 correspondent au résultat de bruteforce (rentabilité max, liste action),
        # sans l'action courante
        ...
        if action_selection[1] <= depense_max:
            ...
            # On utilise bruteforce en enlevant le montant de l'action en cours à la dépense max,
            # et on ajoute cette action dans la liste des actions selectionnées
        ...
        # On renvoie à la fin la meilleur rentabilité total,
        # ainsi que la liste des actions et le montant maximum trouvé
